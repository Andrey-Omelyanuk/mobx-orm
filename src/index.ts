...
class UserProfile extends Model {
	...
	@foreignKey(User, 'id') user_id	: number;
}

@store.model
class Message extends Model {
	@foreignKey(User) userA_id: number;
	@foreignKey(User) userB_id: number;
}


// Test composite primary key
// A,B,C is composite primary key
...
class Account extends Model {
	...
	@foreignKey(User)	owner_id: number;
}

@store.model
class BindAccount extends Model {
	@pk id: number;

	@foreignKey(Account, 'A') A_ref1: number;
	@foreignKey(Account, 'B') B_ref1: number;
	@foreignKey(Account, 'C') C_ref1: number;

	@foreignKey(Account, 'A') A_ref2: number;
	@foreignKey(Account, 'B') B_ref2: number;
	@foreignKey(Account, 'C') C_ref2: number;
