...
import { 
    EQ, StringInput, local, Repository, OrderByInput, NumberInput, ArrayStringInput,
...
describe('Query', () => {
    ...
    @local() @model() class A extends Model { @id id: number }
    @local() @model() class B extends Model { @id id: number }

    const repositoryA = A.getModelDescription().repository
    const repositoryB = B.getModelDescription().repository

    afterEach(async () => {
        A.getModelDescription().repository.cache.clear() 
        B.getModelDescription().repository.cache.clear() 
        jest.clearAllMocks()
    })

    describe('Constructor', () => {
        it('default', async ()=> {
            const query = new Query<A>({repository: A.getModelDescription().repository})
            expect(query).toMatchObject({
                repository      : A.getModelDescription().repository,
                filter          : undefined,
    ...
    describe('Destructor', () => {
        it('default', async ()=> {
            const query = new Query<A>({repository: A.getModelDescription().repository}) as any
