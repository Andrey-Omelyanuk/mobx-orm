...
describe('Field: One', () => {
    ...
    it('declare the one with single id', async () => {
        ...
        expect(A.relations['b'].settings.remote_model).toBe((B as any).__proto__)
        expect(A.relations['b'].settings.remote_foreign_ids_names).toEqual(['a_id'])
    })
    ...
    it('declare the one with multi ids', async () => {
        ...
        expect(A.relations['b'].decorator instanceof Function).toBeTruthy()
        expect(A.relations['b'].settings.remote_model).toBe((B as any).__proto__)
        expect(A.relations['b'].settings.remote_foreign_ids_names).toEqual(['a_id1', 'a_id2'])
    })
    ...
    it('cross declare', async () => {
        ...
        expect(A.relations['b_one'].decorator instanceof Function).toBeTruthy()
        expect(A.relations['b_one'].settings.remote_model).toBe((B as any).__proto__)
        expect(A.relations['b_one'].settings.remote_foreign_ids_names).toEqual(['a_id'])

        expect(B.relations['a_one'].decorator instanceof Function).toBeTruthy()
        expect(B.relations['a_one'].settings.remote_model).toBe((A as any).__proto__)
        expect(B.relations['a_one'].settings.remote_foreign_ids_names).toEqual(['b_id'])
    })
