...
import { Model, model, field, many, id } from '../'
...
describe('Field: Many', () => {
    ...
    describe('Declaration', () => {
        ...
        it('declare', async () => {
            ...
            @model({id: id(), a_id: field()})
            class B extends Model {
                id: number
                a_id: number
            }
            model({id: id(), bs: many(B, 'a_id')})(A)

            // many(B, 'a_id')(A, 'bs')
            const modelDescription = A.getModelDescription()
            expect(modelDescription.relations['bs'].decorator instanceof Function).toBeTruthy()
            expect(modelDescription.relations['bs'].settings.remote_model).toBe(B)
            expect(modelDescription.relations['bs'].settings.remote_foreign_id_name).toEqual('a_id')
        })
