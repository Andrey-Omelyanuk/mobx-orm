...
(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    class Input {
        ...
        __doOptions() {
            this.__disposers.push(mobx.reaction(() => this.options.needToUpdate, () => this.__isReady = false));
        }
        __doAutoReset() {
            this.__disposers.push(mobx.reaction(() => !this.options.needToUpdate && !this.disabled, (is_ready) => {
                if (is_ready)
                    this.autoReset(this);
        ...
        __doSyncURLSearchParams() {
            ...
            this.__disposers.push(mobx.reaction(() => this.deserialize(), // I cannot use this.value because it can be a Map
            () => {
                const searchParams = new URLSearchParams(window.location.search);
    ...
    class Query {
        ...
        constructor(props) {
            ...
            mobx.makeObservable(this);
            this.disposers.push(mobx.reaction(() => { var _a; return { isReady: this.isReady, url: (_a = this.repository.adapter) === null || _a === void 0 ? void 0 : _a.getURLSearchParams(this).toString() }; }, mobx.action('MO: Query Base - need to update', ({ isReady }) => { if (isReady)
        ...
        set autoupdate(value) {
            if (value !== this.autoupdate) { // indepotent guarantee
                ...
                if (value) {
                    setTimeout(() => {
                        this.disposerObjects[DISPOSER_AUTOUPDATE] = mobx.reaction(() => this.needToUpdate && this.isReady, (needToUpdate) => {
                            if (needToUpdate)
                                this.load();
                        }, { fireImmediately: true });
